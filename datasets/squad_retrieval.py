...
def convert_squad_to_retrieval(tokenizer, squad_json_train_path = parent_folder_path+"/data_raw/SQuAD/train-v1.1.json", squad_json_test_path = parent_folder_path+"/data_raw/SQuAD/dev-v1.1.json", pickle_save_folder_path = parent_folder_path+"/data_generated", pickle_save_file_path = "squad_retrieval_data", random_seed = 0, num_dev = 2000):
    '''
    ...
    pickle_save_path_complete = pickle_save_folder_path+'/'+pickle_save_file_path+"_seed_"+str(random_seed)+"_dev_"+str(num_dev)+".pickle"
    ...
    # Otherwise generate from scratch and save to pickle
        ...
        with open(squad_json_train_path, "r") as handle:
            squad_json_train = json.load(handle)

        with open(squad_json_test_path, "r") as handle:
            squad_json_test = json.load(handle)
        ...
        for question, q_id, answer_sent, document, para, gold_answer_text in _generate_examples(squad_json_train):
        ...
        squad_retrieval_test = []
        for question, q_id, answer_sent, document, para, gold_answer_text in _generate_examples(squad_json_test):
            if document in seen_documents_dict:
                document_id = seen_documents_dict[document]
            else:
                document_id = sent_doc_resp_count["doc"]
            ...
            squad_retrieval_test.append(question_dict_to_append)
        ...
        with open(pickle_save_path_complete, "wb" ) as handle:
            pickle.dump({"train_list": squad_retrieval_train,
                         ...
                         "test_list":squad_retrieval_test,
                         "sent_list":sent_list,
                         "doc_list":doc_list,
                         "resp_list":response_list,
                         "sent_list_raw":sent_list_raw,
                         "doc_list_raw":doc_list_raw}, handle)

    return {"train_list": squad_retrieval_train,
                         "dev_list": squad_retrieval_dev,
                         "test_list":squad_retrieval_test,
                         "sent_list":sent_list,
                         "doc_list":doc_list,
...
def check_squad_retrieval_pickle(saved_pickle_path = parent_folder_path+"/data_generated/squad_retrieval_data_seed_0_dev_2000.pickle"):
    ...
    for data_partition in ["train_list", "dev_list", "test_list"]:
        non_standard_example = 0
        print("="*20+"\n"+"checking data partition "+data_partition)
        print("number of samples in this partition:", len(squad_retrieval_data[data_partition]))

        samples_to_check = random.sample(list(range(len(squad_retrieval_data[data_partition]))), 10)
        for i, question_dict in enumerate(squad_retrieval_data[data_partition]):
            try:
            ...
            assert(squad_retrieval_data["resp_list"][question_dict["response"]][0]==question_dict["answer"])
            assert(squad_retrieval_data["resp_list"][question_dict["response"]][1]==question_dict["document"])

            if i in samples_to_check:
                print("-"*20)
                print("\tsample index:", i)
                print("\tquestion:", question_dict["question"])
                print("\tanswer sent reconstruct:", squad_retrieval_data["sent_list_raw"][
                    squad_retrieval_data["resp_list"][question_dict["response"]][0]])
                print("\tanswer doc reconstruct:", squad_retrieval_data["doc_list_raw"][
                    squad_retrieval_data["resp_list"][question_dict["response"]][1]])
                print("\tanswer sent ids reconstruct:", squad_retrieval_data["sent_list"][squad_retrieval_data["resp_list"][question_dict["response"]][0]])
                print("\tanswer doc ids reconstruct:", squad_retrieval_data["doc_list"][squad_retrieval_data["resp_list"][question_dict["response"]][1]])
